...
In a nutshell, our goal is to fit a set of models to a set of datasets; members of each of these sets are enumerated below.  
...
In all cases, we optimize parameter values to minimize the sum of squared error between model and human performance.
We use the DEoptim R package [@DEoptim] to optimize parameters via differential evolution.
...
Second, we fit each model to all conditions and datasets to produce a single best-fitting parameter set per model. 
...
Specifically, greater attention is directed to pairings that have previously co-occurred (a familiarity bias), but is also directed to stimuli that have no strong associates, tracked via the entropy of their association strengths (e.g., novel stimuli, or stimuli that have diffuse associations with several stimuli). 
...
Group fits are presented in Table 

Correlations between data and models are presented in Figure. 
```{r load-cv-fits}
