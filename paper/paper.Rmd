...
Our primary outcome measure is the correlation between model average performance for each item in a specific experimental condition and item-level human performance on that same experimental condition. 
...
This sampling version of the Kachergis et al. model thus can still build multiple graded associations for each word (or object), but on any given run will accumulate only a sparse, randomized version of what the full associative model would build.
...
The *strength-biased model* lacks the uncertainty bias term, and thus only implements a bias for familiar (i.e., already-strong) associations, with learning rate ($\chi$) and decay $\alpha$ parameters.

The *uncertainty-biased model* lacks the strength bias term, and thus only implements a bias for stimuli with uncertain (high-entropy) associations.
The *novelty-biased model* lacks the strength bias term, and substitutes novelty for the entropy terms (e.g., 1/(frequency(*w*)+1) instead of $H(w)$.
These models still have all three parameters of the original Kachergis et al. model, although $\chi$ and $\beta$ likely trade off.
...
However, data from several conditions have never been published, but were shared by Chen Yu's lab at Indiana University.
...
Table 1 shows the sum of squared error (SSE) and correlation ($r$) for each model's best-fitting parameters vs. average human performance on the `r totItems` items, both for the CV fits and for the group fits, along with the number of fitted parameters in each model (P).
For comparison, the 0-parameter baseline co-occurrence model had SSE = 32.0 and $r = 0.53$.
...
```{r 2-col-image, fig.env = "figure*", fig.pos = "h", fig.width=7, fig.height=6.5, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Human vs. model performance with a single best-fitting set of parameters for all conditions."}
...
Restate general goal in a few sentences

Pursuit et al. doesn't work. 

Fazly and associative both do quite well - why? what's similar what's differnet
correlations between those, where do they diverge maximally (what condition)? 
how could we design an experiment to decide? 
In three cross-situational learning experiments, Hendrickson gave participants word-object pairs occurring with Zipf (power-law) frequency distribution, and then compared two models: Fazly and Kachergis.
