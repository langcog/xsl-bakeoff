...
abstract: >
    ...
    occurrences of a given word under many experimental manipulations. 
    There has been correspondingly much interest in modeling such learning, 
    and yet there is little consensus on the main mechanisms underpinning
    cross-situational word learning. The lack of consensus is in part due 
...
# abstract is a bit long: 188 words
---
...
[^1]: The original model implemented a $\theta$-thresholded lexicon that discretized the probability matrix, but we found that including this mechanism (and additional parameter) resulted in a worse fit to the data.
...
### Strength- (S), Uncertainty- (U), and Novelty-Biased (N) Models
...
These models still have all three parameters of the original Kachergis model, and at test operate in the same way as the original model. <!--, although $\chi$ and $\beta$ likely trade off.-->
...
At test, for each word the model chooses the currently hypothesized referent for that word.
<!-- , or chooses at random if no referent is stored.
...
At test, this model chooses the best referent for each word by Luce choice from the lexicon, which may or may not have multiple referents associated with each word.
...
group_fits$fazlyt = NULL # thresholded Fazly model

group_fits$pursuit_detailed = NULL # old pursuit version

group_fits$trueswell2012$optim$bestmem = c(0.41895326, 0.02030728) # SSE=.00239
group_fits$pursuit$optim$bestmem = c(0.119080, 0.692228, 0.000502) # SSE=.0039
group_fits$`guess-and-test`$optim$bestmem = c(0.4670804, 0.9600860) # SSE=.0028
...
mod_names_short = c("Ks","PA","FU","N","U","BD","RW","S","PV","GT","P")
rownames(mod_cors) = mod_names_short
colnames(mod_cors) = mod_names_short
```

How related to each other are these models?
Table 2 shows the correlations between model predictions made using the group-optimized parameters. 
...
Using the optimal parameters found for each model in the group fit, we simulate model performance for two other published adult studies and two developmental studies for which we only have participants' average performance. 
...
@Medina2011 manipulated the informativeness (number of referents per trial) and order of trials across four conditions, again with adult learners.
...
@Suanda2012 tested X-year-old children in three conditions with varying contextual diversity.

```{r gen-exp-table, results="asis"}
gen_tab <- xtable::xtable(gen_tab,  caption = "Generalization experiment results.")

#print(tab3, type="latex", comment = F, include.rownames = T, table.placement = "H")
kable(gen_tab, "latex", booktabs = T) %>%
  kable_styling(latex_options = "scale_down")
# ToDo: bold minimum per exp (column)
```
...
The predictions of the top three models are more correlated with each other than with the human data (see Table 1): Fazly vs. Kachergis sampling: .87, Fazly vs. Kachergis: .84, Kachergis vs. sampling: .92.
...
We enthusiastically invite other researchers to contribute datasets and model implementations for future, larger-scale comparisons.
