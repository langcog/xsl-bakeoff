...
author-information: > 
    \author{{\large \bf George Kachergis (kachergis@stanford.edu)} \and 
    \author{{\large \bf Kevin Liang (.edu)} \and
    {\large \bf Michael C.~Frank (mcfrank@stanford.edu)} \\ Department of Psychology, 450 Jane Stanford Way \\ Stanford, CA 94305 USA}
...
# abstract is a bit long: 152 words
---
...
Following earlier work, we first evaluate on small collections of annotated transcripts from the @rollins2003caregivers corpus, annotated by Frank, Goodman, & Tenenbaum (2009), and on transcripts from 24 English-speaking infants from Fernald & Morikawa (1993), annotated by @frank2013social. 
...
They are also limited in size when compared to the large body of child-directed corpora that is available in CHILDES \cite{CHILDES}. 
...
Two different metrics are used to evaluate the models. The first of these metrics is the $F_1$ score. As the standard metric in evaluating word-learning models, the $F_1$ score is used to facilitate comparison with the literature \cite{Stevens2017}. The  $F_1$ score is the harmonic mean of two other metrics - precision and recall. Precision measures the percent of correct predictions out of the total number of predictions (i.e., out of all the word-referent pairs proposed, what proportion are correct?) whereas recall measures the number of correct predictions by the model out of all the possible correct predictions (i.e., out of all the word-referent pairs that can be learned, what proportion did the model learn?)  In this case, we use the provided "gold" lexicon for each corpus - a list of word-referent mappings that have been judged to be learnable - to determine the correct word-referent mappings.

The second metric is the comprehension score \cite{fazly2010}. This score is defined as the sum of the probabilities of each referent being matched with the correct word (i.e., the expected value of the number of correct word-referent pairs) and allows us to better understand how learning takes places over time for each model. Additionally, it can still be used even when there is no "gold" lexicon present, and it is not obvious which mappings are learnable (i.e., in the case of the generated corpora). 

Although the corpora used to train each model is identical, each model also has a set of unique parameters that need to specified. Differing settings for these parameters can significantly affect the performance of the model. Consequently, for a fair comparison of these models, we fit these parameters to the data. For all models, we perform an optimization to maximize the $F_1$ score of the model. This optimization was done using differential evolution with the DEoptim package \cite{DEoptim} in R. Due to the stochastic nature of the hypothesis-testing models considered, we simulated a sample of 500 learners to optimize the parameters for those models. 
