...
output: pdf_document
author-information: |
  \author{{\large \bf George Kachergis (kachergis@stanford.edu)} \and  \author{{\large \bf Kevin Liang (.edu)} \and {\large \bf Michael C.~Frank (mcfrank@stanford.edu)} \\ Department of Psychology, 450 Jane Stanford Way \\ Stanford, CA 94305 USA}
abstract: "One problem language learners face is extracting word meanings from  scenes
  with many possible referents. Despite the ambiguity of individual    situations,
  a large body of empirical work shows that people are able to learn cross-situationally
  ...
  theories of word learning, find two   best-fitting models, and discuss issues of
  model and mechanism identifiability.  Finally, we test the models' ability to generalize
...
Yet at its heart, these information sources can be seen as reducing the uncertainty inherent in the task of *cross-situational mapping* [@Frank2009]: that is, cross-referencing which referents a word has consistently appeared with across multiple uses [@Gleitman1990; @Carey1978]. This word learning task is referred to as cross-situational learning.
...
In such experiments, participants are typically presented with a series of training trials comprised of multiple possible referents (e.g., an array of 2-4 novel objects) and one or more spoken nonce words,
<!-- G: maybe add what "nonce words" mean in this context: words that do not correspond to any meaning? -->
and (when possible) instructed to learn the meaning(s) of the presented words.
...
<!-- G: that felt a bit brief. It might make sense to mention towards which direction and what numbers. Especially if this is going to be discussed later on. -->
...
Identifying the model(s) that best predict human performance is an important goal for consolidating this broad literature. 
...
However, while most models of cross-situational learning have typically been tested against at least a few datasets, to our knowledge there has been no systematic comparison of models across a wide variety of experimental conditions. 
Different models also often contain a range of parameters (e.g., a learning rate) that should be optimized to fit the observed data. 
Previous comparison work was thus hindered, either because the parameters have been allowed to vary freely across multiple conditions, or because models' performance is compared on different datasets. 
...
Thus, the goal of our current study is to test the ability of different cross-situational word learning models in accounting for a wide range of experimental data, by systematically comparing the performance of a range of models.
<!-- G: maybe something like: "Thus, the goal of our current study is to systematically compare a range of cross-situational word learning models and evaluate their viability. We do so by testing the models' ability in accounting both for a wide range of experimental and naturalistic data." -->
We do so by testing the models' ability in accounting both for a wide range of experimental data and for naturalistic child-directed speech data. 
Our goal is to scale up both the number of models evaluated, the number of experiment designs (and participants) and the nature of the data (both lab, and real life)
being used for evaluation. 
We undertake this challenge by using cross-validated parameter fits and focusing on out-of-sample prediction of human data for a range of cross-situational models.
...
<!-- Georgia: mention the package at this point? --> 
...
Each model typically is defined as a function that takes input data  and then can be evaluated on test trials (typically multi-alternative forced choice test). 
...
Although input data and test trials are features of specific experimental tests, each model also has a group of parameters (often between 2--4) that must be specified. 
...
For all models, we optimize parameter values to minimize the sum of squared error (SSE) between model and human performance, weighting the contribution of each condition's SSE by the number of participants in that condition.
...
We conduct two evaluations of models. 
First, we optimize each model's parameters on 80% of data and evaluate on the remaining 20% (5-fold cross-validation).
Second, in the *group fit* regime we fit each model to all conditions and datasets (i.e., optimizing each model's parameters on 100% of data) and evaluate across all data. 
Though slightly overfitting may occur in the *group fit* regime, this evaluation allows us to make the best test of generalization to other experiments from the literature. 
...
<!-- Georgia: maybe more info about these categories in general, what the hybrid models are, and a reference? --> 
...
### Probabilistic Associative Model (PA)
...
### Familiarity- and Uncertainty-biased Model (FU)
...
### Strength-, Uncertainty-, & Novelty-biased (Str,Unc,Nov) Models
We also test three variations of the FU model that use only a subset of the mechanisms of the full model in order to understand the contributions of each mechanism.
...
The *novelty model* also lacks the strength bias term, and substitutes novelty for the entropy terms (e.g., 1/(frequency(*w*)+1) instead of $H(w)$).
These models have all three parameters of the original FU model, and operate in the same way as that model at test. <!--, although $\chi$ and $\beta$ likely trade off.-->
...
### Propose-but-Verify Model (PbV)
In the propose-but-verify hypothesis testing model [@Trueswell2013], a presented referent is selected at random for any presented word that has no remembered referent. 
The next time that word occurs, the previously-proposed referent is remembered with probability $\alpha$, a free parameter. 
If the remembered referent is verified to be present, the future probability of recalling the word is increased by parameter $\epsilon$. 
...
### Pursuit Model (Pur)
The pursuit model [@Stevens2017pursuit] is a hybrid model, potentially storing graded associations involving a given word and multiple referents (or vice-versa), while on any given trial greedily pursuing the  strongest association for each presented word.
...
## Data
<!-- Georgia: "Experimental data" maybe? To differentiate with the corpus data below?-->
...
We give a brief overview of included conditions, and the full  important takeaway is that within this dataset, there is substantial variation along each of these dimensions, making it a useful testbed for establishing the generality of word learning models.
A table containing design details of each condition is available on OSF[^2], and the trial-by-trial order information for all `r length(combined_data)` conditions -- along with participants' item-level average performance -- is included in our R package [ToDo: github XSLmodels].
...
The bulk of these data have been previously published. 
For example, data from 13 of the conditions were reported in @Kachergis2013nat, which compared performance on conditions with differing numbers of words and objects presented per trial (e.g., 1 word and 3 objects per trial; dubbed "1x3") to those with equal numbers of words and objects per trial (e.g., "2x2", "3x3", and "4x4"), with an equal amount of overall repetitions per stimulus.
Data from another 12 conditions are from @Kachergis2012gi, and data from another nine conditions are from @Kachergis2016, which manipulated frequency and contextual diversity word-object pairs.
However, data from several conditions are contributed by Chen Yu's lab at Indiana University, 
<!--G: X participants out of total -->
...
[^2]: Information about the dataset: [https://osf.io/REDACTED](https://osf.io/s2aty/?view_only=c91c852f00b847e0a71d5f4deaf7b7a5)

# Results
<!-- Georgia: maybe "Experimental results" ? -->
...
cvg_fit_tab <- cvg %>% group_by(Model) %>% 
  ...
  rename(`All SSE` = SSE) %>%
  mutate(Model = case_when(Model == 'kachergis' ~ 'FU',
                             Model == 'fazly' ~ 'PA',
                             Model == 'novelty' ~ 'Nov',
                             Model == 'Bayesian_decay' ~ 'BD', 
                             Model == 'strength' ~ 'Str',
                             Model == 'uncertainty' ~ 'Unc',
                             Model == 'rescorla-wagner' ~ 'R-W',
                             Model == 'trueswell2012' ~ 'PbV',
                             Model == 'guess-and-test' ~ 'GnT',
                             Model == 'kachergis_sampling' ~ 'FUs',
                             Model == 'pursuit' ~ 'Pur',
                             #Model == 'baseline' ~ 'B',
                          TRUE ~ Model))

# ToDo: add baseline?
# "group fit": baseline 31.8 0.617 (would be in 10th place, ahead of strength and pursuit)

# ToDo: use case_when to make this robust to reordering
#cvg_fit_tab$Model = c("FUs", "PA", "FU", "Nov", "Unc", "BD", "GnT", "R-W", "Str", "Pur", "PbV") 
cvg_fit_tab$P = c(3, 2, 3, 3, 3, 3, 2, 3, 2, 3, 2)
```
...
The results of the two fitting procedures mostly yielded similar SSEs and correlations, and a consistent rank-ordering of the models, with the Kachergis sampling model performing best, followed closely by the Fazly et al. model and then the associative Kachergis et al. model.
The hypothesis testing models (Propose-but-Verify and Guess-and-Test), the Pursuit model, and R-W fit roughly as well as the 0-parameter baseline co-occurrence counting model, which had SSE = 32.0 and $r = 0.53$.
...
mod_names <- setdiff(group_fit_tab$Model, "baseline")
...
diag(mod_cors) = 0
mod_names_short = c("FUs","PA","FU","Nov","Unc","BD","PbV","GnT","R-W","Str","Pur")
...
# ToDo: maybe also add r or MSE to each plot
...
gen_tab <- gen_exps %>% mutate(RMSE = sqrt(MSE)) %>% select(-MSE) %>%
  ...
  rename(Model = model,
         ...
         Medina = `Medina et al.`,
         Suanda = `Suanda et al.`) %>%
  mutate(`Adult RMSE` = rowSums(.[2:3]),
         `Dev RMSE` = rowSums(.[4:5])) %>% 
  arrange(`Adult RMSE`) %>% 
  mutate(Model = case_when(Model == 'kachergis' ~ 'FU',
                             Model == 'fazly' ~ 'PA',
                             Model == 'novelty' ~ 'Nov',
...
Using the optimal parameters found for each model in the group fit, we simulated model performance for four other published (two adult, two developmental) for which we only have participants' average performance. 
<!-- Georgia: what's the difference with the ones above? what are developmental corpora? -->
...
@Smith2008vg and @yu2011you trained 12- and 14-month-old infants on 6 words repeated across 30 trials, and found that infants learned on average 4 words.
@Suanda2014 tested 6-year-old children in three conditions with varying contextual diversity.
Table 3 shows the root-mean-square error (RMSE) of each model vs. average human performance, as well as the total RMSE separately for the adult experiments and the developmental experiments.
The models generally outperformed children and thus did not fit well to the developmental conditions.
Nevertheless, the Rescorla-Wagner model best matched both the Medina et al. adult data and the Smith & Yu developmental data.
The novelty-biased model best predicted the Koehne et al. adult data, with FUs and the Strength-biased models just behind.
Finally, the Suanda et al. developmental data was best fit by the Bayesian Decay model. 


```{r gen-exp-table, results="asis"}
#gen_tab$model = c("R-W", "Nov", "FUs", "Str", "PA", "Pur", "", "PbV", "GnT", "Pur", "BD")
...
for(c in 2:ncol(gentab)) {
  ...
  #if(c!=ncol(gentab)) 
  gentab[min_row,c] = cell_spec(gentab[min_row,c], "latex", bold = T)
}
# ToDo: add separate Total RMSE column for adult and devo data
kable(gentab, "latex", booktabs = T, digits=2, escape=F, caption = "Generalization experiment results.") #%>%
  #kable_styling(latex_options = "scale_down")
```

```{r}
rw <- subset(gfd, Model=="rescorla-wagner")
b <- subset(gfd, Model=="baseline")
RW_vs_baseline = cor(rw$ModelPerf, b$ModelPerf) # .999 -- but not 1!
```


```{r models-diverge}
# find conditions where the best-performing models maximally diverge
mw <- gfd %>% mutate(ID = rep(1:726, length(unique(gfd$Model)))) %>%
...
Now we turn to evaluating these models on selected corpora of child-directed speech.
Following earlier work, we first evaluate on small collections of annotated transcripts from the @rollins2003caregivers corpus, annotated by Frank, Goodman, & Tenenbaum (2009), and on transcripts from 24 English-speaking infants from @fernald1993common, annotated by @frank2013social. 
<!-- mention ages of children in Rollins (9 mos, 12 mos, and 30 mos, but what ages were transcribed?) and FM (6, 12, and 19 mos), and number of utterances (and unique nouns, maybe?) -->
For each utterance in these corpora (referred to as FGT and FM, respectively), researchers viewed the video recordings and annotated which referents were in visible to the child at the time of the utterance.
For example, the utterance "it's a look and see book" is tagged with "book, bird, rattle, face" as those were the referents visible in the scene during the utterances. 
As seen in Table \ref{fig:corporastats}, the number of utterances in the FM corpus is almost eight times greater than the number of utterances in the FGT corpus, and the number of word types is almost three times greater. 
However, there is a comparable number of referent types. 
Thus, the FM corpus represents a much noisier learning environment which allows for a comparison of the models in different contexts.

\begin{table}[h!]
\caption{\label{fig:corporastats} Corpora Statistics}
\vskip 0.12in
\centering
\scalebox{0.9}{
    \begin{tabular}{ |c|c|c|c| }
        \hline
         Corpus  & Word Types & Referent Types & Number of Utterances  \\
        \hline
         FGT  & 419 & 22 & 624 \\
         FM & 1122 & 30 & 4763 \\
         \hline
...
Finally, we also investigate model performance on another corpus selected CHILDES, for which no referent annotations were available.
If the results are largely concordant with the above results, we suggest that additional model testing and learnability analyses can usefully be done using just these scenes constructed from audio transcripts.

\subsection{Corpus Generation Procedure}


Although the combined size of the FGT and FM corpora are larger than most corpora used to test cross-situational word learning models in the literature, their utility is limited by the fact that both corpora are of similar contexts (play sessions) and only contain English-speaking children. 
They are also limited in size when compared to the large body of child-directed corpora that is available in CHILDES [@MacWhinney2000]. 
Through the use of generated corpora, we are able to perform further comparisons on both larger and more diverse datasets that are not possible with the FGT and FM corpora. 

Thus, we propose a method of automatically annotating CHILDES corpora to include referents present in the scene. Specifically, we group every $\alpha$ utterances together (where $\alpha$ is a free parameter) and all the nouns (or potentially other specified syntactic categories) are presumed to be present in the scene. This assumption is a drastic oversimplification of naturalistic learning situations since there are often referents which are not uttered that are present and there are words which are uttered that do not have a corresponding referent. Nevertheless, we will show that such a simplistic method can create corpora that demonstrate many of the same properties as the FGT and FM corpora when used as inputs to word learning models. 

First, we examine how well the models perform on the FGT and FM corpora using the best-fitting parameters from the adult experiments.
Then we optimize the models' parameters for the FGT and FM corpora, and discuss any broad differences between using the experiment-optimized parameters and the corpus-optimized parameters. 
Finally, we test the generality of our results with respect to scenes constructed form audio-only transcripts of child-directed speech.

### Experiment-optimized Parameters

Two different metrics are used to evaluate the models. 
The first of these metrics is the $F_1$ score, a standard metric in evaluating the accuracy of classification models that is harmonic mean of precision and recall. 
*Precision* is the number of correct predictions out of the total number of predictions (i.e., out of all the word-referent pairs proposed, what proportion are correct?), whereas *recall* is the number of correct predictions by the model out of all the possible correct predictions (i.e., out of all the word-referent pairs that can be learned, what proportion did the model learn?). 
In this case, we use the provided "gold" lexicon for each corpus -- a list of word-referent mappings that have been judged to be learnable -- to determine the correct word-referent mappings.

The second metric is the *comprehension score* \cite{fazly2010}, defined as the sum of the probabilities of each referent being matched with the correct word (i.e., the expected value of the number of correct word-referent pairs).
The comprehension score allows us to better understand and compare how learning takes places over time for each model. 
Additionally, it can still be used even when there is no "gold" lexicon present, and it is not obvious which mappings are learnable (i.e., in the case of the generated corpora). 

Although the corpora used to train each model are identical, each model also has a set of unique parameters that need to specified. 
Differing settings for these parameters can significantly affect the performance of the model. Consequently, for a fair comparison of these models, we fit these parameters to the data. 
For all models, we perform an optimization to maximize the $F_1$ score of the model. 
This optimization was done using differential evolution with the DEoptim package \cite{DEoptim} in R. 
Due to the stochastic nature of the hypothesis-testing models considered, we simulated a sample of 500 learners in each iteration of parameter optimization for those models. 

Subsequently, we use these optimized parameters to test whether the models show any order effects. In other words, is the performance of each model affected by the order in which the utterances are presented?

<!-- can pull more from: https://www.overleaf.com/project/61c2da4a85034543cfe10bec -->

```{r}
# show model performance based on parameters fitted to above experiments
# ToDo: sampling models ?
```


### Corpus-optimized Parameters

<!-- maybe optimizing for short experiments is unfair: which models can do much better if optimized?  --> 
<!-- noun bias vs. verb? only if it's counterintuitive wrt the statistics (if 1 verb/sentence, makes sense) --> 

As we show in Appendix X, similar results are found for audio-only corpora where we generate


### Generalization to Audio-only Corpora
<!-- ToDo: move to appendix? -->
<!-- use a longitudinal corpus? e.g. Manchester? would be interesting if there are predicted individual differences -->



# Discussion

We set out to conduct a systematic comparison of several models of the cross-situational word learning models described above across a wide variety of experimental conditions. 
Using a large dataset, we found best-fitting parameters for 11 models both by simultaneously fitting all of the data, and using cross-validation.
The results of both fitting regimes were consistent and clear: shown in Table 1, the associative models generally achieved better fits (lower SSE and higher correlations) than the hypothesis-testing models (GnT; @Medina2011 and PbV; @Trueswell2013), which made correlated predictions (see Table 2: $r=.65$).
The Rescorla-Wagner model, a prediction error-based associative model, performed similarly to the GnT model, with highly correlated predictions ($r=.87$), while PbV had the poorest fit, and made responses most similar to the R-W model ($r=.67$).
Moreover, the predictions of the R-W model were also highly correlated with the baseline co-occurrence counting model ($r = .999$): such similar predictions yielded by seemingly different theories is surprising and warrants further investigation.
However, some models' particular patterns of responses suggest certain theoretical shortcomings.
For example, the hybrid Pursuit model [@Stevens2017pursuit] fared poorly, in part because it often outperformed humans ($M_{Pur} =$ `r round(mean(subset(gfd, Model=="pursuit")$ModelPerf), 2)`), $M_{hum} = 0.48$).
Indeed, as shown in Figure 1, Pur, PbV, and GnT all showed a pattern of inhuman responses: for a subset of items, the models either always got them correct or always got them incorrect, while people were more stochastic.
<!-- And yet Pur made responses that were most highly correlated with the FUs model (r=.74)--one of the best-fitting models. -->

Among the associative models, the PA model [@Fazly2010] and the FU model [@Kachergis2012gi] both fare quite well, and the sampling version of the FU model (FUs) performs slightly better, albeit with three free parameters compared to two in the PA model.
...
Future work should aim to further explore when these models make overlapping predictions, but another approach is to find conditions where models make disparate predictions. 
The FUs and PA models make maximally divergent predictions in the conditions with fewer words than objects per trial (esp. 1x3, 1x3 6/18 4AFC, and 1x4), which hints at a difference in mechanism for these models. 
A fruitful next step would be to use optimal experiment design to generate a definitive experiment to differentiate these models. 
...
First, there are many other models that we have not yet implemented [e.g., @mcmurray2012word]. <!-- or that we have not finished fitting to the data [e.g., @YurovskyFrank2015].  @tilles2012minimal; -->
Moreover, this dataset represents performance from a fairly homogeneous sample: English-speaking US college students. 
...
The small number of generalization experiments run here suggest that parameters fitted to adult experimental conditions do not generalize well to developmental experiments: future research should gather a larger set of developmental data and re-fit these models with cross-validation.
We invite other researchers to contribute datasets and model implementations for future, larger-scale comparisons: data and model code will be open-sourced and available on [OSF](https://osf.io/s2aty/?view_only=c91c852f00b847e0a71d5f4deaf7b7a5).
...
More broadly, we believe that structured model comparison is a critical step towards developing more complete models and, eventually, quantitative identification of the fundamental mechanisms of word learning.
...
We thank members of the Language and Cognition Lab at Stanford University for feedback on early versions of this manuscript.
