...
run_pretrain_experiment_batch <- function(parms, n_pretrained=3, start_val=1, Nsim=100) {
  dat = data.frame()
  for(i in 1:Nsim) {
...
evalSSE_Suanda <- function(model_name, par, human, verbose=F) {
  source(paste0("models/",model_name,".R"))
...
#evalSSE_Suanda(c(0.044, 2.840, 0.999), suanda_emp, verbose=T) # SSE=.004
# 0.38 0.41 0.43

# adult params get high performance
#evalSSE_Suanda( c(0.490592, 6.927328, 0.802789), human, verbose=T) 

for(model_name in names(group_fits)[1:6]) {
  pars = group_fits[[model_name]]$optim$bestmem
  sse = evalSSE_Suanda(model_name, pars, suanda_emp, verbose=F) # 
