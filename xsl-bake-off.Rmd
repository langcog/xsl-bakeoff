...
for(model_name in names(group_fits)) {
  ...
  mse = evalMSE_Koehne(model_name, pars, Koehne, verbose=F) # 
  gen_exps = bind_rows(gen_exps, tibble(exp="Koehne et al.", model=model_name, MSE=mse))
}
...
run_Medina_exp <- function(model_name, pars, plot=F) {
  ...
  for(i in 1:length(Medina)) {
    ...
    df <- bind_rows(df, bind_cols(Condition = rep(names(Medina)[i],5),
                              Vignette = 1:5,
                              ModAcc = get_block_perf(mp),
                              HumAcc = Medina[[i]]$HumBlockAcc))
  }
  df$Model = model_name
...
evalMSE_Medina <- function(model_name, pars) {
  ...
  return( sum((df$HumAcc - df$ModAcc)^2) / nrow(df) )
}
...
for(model_name in names(group_fits)) {
  pars = group_fits[[model_name]]$optim$bestmem
  mse = evalMSE_Medina(model_name, pars) # 
  gen_exps = bind_rows(gen_exps, tibble(exp="Medina et al.", model=model_name, MSE=mse))
}
...
evalMSE_orders <- function(model_name, par, orders, verbose=F) {
  if(is.element(model_name, stochastic_models)) {
    source(here(paste0(model_dir,"stochastic/",model_name,".R")))
...
for(model_name in names(group_fits)) { 
  pars = group_fits[[model_name]]$optim$bestmem
  mse = evalMSE_orders(model_name, pars, Suanda, verbose=F) # 
  gen_exps = bind_rows(gen_exps, tibble(exp="Suanda et al.", model=model_name, MSE=mse))
}
...
# sum(mAFC_test(mp$matrix, 2))

for(model_name in names(group_fits)) { 
  pars = group_fits[[model_name]]$optim$bestmem
  mse = evalMSE_orders(model_name, pars, smith_yu, verbose=F) # 
  gen_exps = bind_rows(gen_exps, tibble(exp="Smith & Yu", model=model_name, MSE=mse))
}

save(gen_exps, file=here("fits/generalization_exp_results.RData"))
```
