...
Medina[["HIlast"]]$HumAcc = c(.07, .09, .14, .08, .21) # final: .09
Medina[["HIabsent"]]$HumAcc = c(.16, .20, .21, .18, .21) # final: .18
...
for(model_name in names(group_fits)) {
  pars = group_fits[[model_name]]$optim$bestmem
  #sse = evalSSE_Medina(model_name, pars, Koehne, verbose=F) # 
  #gen_exps = bind_rows(gen_exps, tibble(exp="Medina et al.", model=model_name, SSE=sse))
}
...
for(model_name in names(group_fits)[1:6]) { # [1:6]
  pars = group_fits[[model_name]]$optim$bestmem
